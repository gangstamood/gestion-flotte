...
if page == "📊 Dashboard":
    ...
    if vh_srv:
        ...
        if 'dernier_bon' in st.session_state:
            ...
            # Affichage avec composants Streamlit natifs au lieu de HTML
            st.markdown("### 📄 BON DE CARBURANT")
            st.markdown("---")
            
            col1, col2 = st.columns(2)
            with col1:
                st.write(f"**N° :** {bon['numero_bon']}")
                st.write(f"**Véhicule :** {bon['immatriculation']}")
                st.write(f"**Service :** {bon['service']}")
            with col2:
                st.markdown(f"<p style='font-size: 18px; color: #d9534f;'><strong>Carte N°{bon['numero_carte']}</strong></p>", unsafe_allow_html=True)
                st.write(f"**Date :** {bon['date']}")
                st.write(f"**Conducteur :** {conducteur_prenom} {conducteur_nom}")
            
            if bon.get('notes'):
                st.write(f"**Notes :** {bon['notes']}")
            
            st.markdown("---")
            st.markdown("*Volume, type et montant à saisir au retour*")
            st.markdown("---")
